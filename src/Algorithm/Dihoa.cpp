...
Dihoa::Dihoa(EnvironmentPtr env) {
  ...
  env_->TaskQueue->addTask(task_solve_dual_single_tree, "t_solve_dual_single_tree");
  ...
  env_->TaskQueue->addTask(task_check_hybrid_event, "t_check_hybrid_event");
...
bool Dihoa::run() {
  ...
  try {
    while (!env_->TaskQueue
                ->isTaskQueueEmpty()) {// todo : check if it works with MPI
      for (const auto &task : env_->TaskQueue->tasks_) {
        if (task.first->isActive()) {
          ...
          if (env_->TaskQueue->isTaskQueueEmpty()) {
            break;
          }
        }
