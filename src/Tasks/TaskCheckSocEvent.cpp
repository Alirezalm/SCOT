...
void TaskCheckSocEvent::execute() {
  ...
  auto target_task_gather_quad_oa = env_->TaskQueue->getTask("t_gather_quad_oa");
  auto target_task_gather_linear_oa = env_->TaskQueue->getTask("t_gather_lin_oa");

  auto target_task_add_quad_oa = env_->TaskQueue->getTask("t_add_quadratic_oa_cut");
  auto target_task_add_linear_oa = env_->TaskQueue->getTask("t_add_linear_oa_cut");
